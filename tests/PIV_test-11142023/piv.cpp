...
void* Signal(void* arg){
    ...
    while(true) {
        ...
        delayMicroseconds(flash * 1000); // g
        digitalWrite(cameraPin, 0); // Falling edge
        delayMicroseconds(exp_flash * 1000); // wait delt_t for the second signal
        digitalWrite(laserPin, 1);
...
void* pivGrab(void* arg){
    ...
    //std::cout<< camera.PixelFormat.GetValue() << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    */
    ...
    // Create new threads;
    int params_signal[5] = {exposure, dur, freq, dt, laser_on};
